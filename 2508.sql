...
from Book B, Author A, BookAUthor BA
Where B.ISBN=BA.ISBN and BA.AUthorID=A.AurthorID
Select B.Title, M.Name, BW.BorrowDate
From Book B, Member M, Borrow BW
where M.MemberID=BW.MemberID and BW.ISBN=B.ISBN
Select A.Name, B.Title
From Book B, Author A, BookAuthor BA
where BA.AuthorID=A.AurthorID and BA.ISBN=B.ISBN
Select M.Name, Count(BW.BorrowID)
...
From Member M, Borrow BW
where M.MemberID=BW.MemberID
group by M.Name having count(BW.BorrowID)>1
Select year(BW.BorrowDate)as Year, count(B.Title) as 'Books Borrowed'
From Book B, Borrow BW
where B.ISBN=BW.ISBN
group by year(BW.BorrowDate)
Select M.Name
from Member M, BookAuthor BA, Author A, Borrow BW
where A.Name='Leo Tolstoy' and A.AurthorID=BA.AuthorID and BA.ISBN=BW.ISBN and M.MemberID=BW.MemberID;
select B.Title, count(BA.AuthorID)
From Book B, BookAuthor BA, AUthor A
where B.ISBN=BA.ISBN and BA.AuthorID=A.AurthorID
group by B.ISBN,B.Title Having count(BA.AuthorID)>1
...
select M.Name, B.Title
From Member M, Book B, Borrow BW
where M.MemberID=BW.MemberID and B.ISBN=BW.ISBn and year(B.YearOfPublication)<2000
select A.Name, count(BA.AuthorID)as C
from Author A, BookAuthor BA, Book B
where B.ISBN=BA.ISBN and BA.AuthorID=A.AurthorID 
group by A.Name
Order By C Desc

insert into Member values(1111, 'Ian', 'Kanjurmarg', '1928374617','2023-01-01') 
select M.Name
From Member M, Borrow BW
where M.MemberID Not in(BW.MemberID)

Select AVG(CountBorrow))
from (select M.MemberID, count(*) as CountBorrow from Member M, Borrow BW)
group by M.MemberID
...
Inner Join Author A ON BA.AuthorID=A.AurthorID;

Select M.*
From Member M
Left Join Borrow B ON M.MemberID=B.MemberID Where B.BorrowID IS NULL;

Select * From Book Where ISBN Not in (Select Distinct ISBN From Borrow);

Select B1.* From Borrow B1
WHERE BorrowDate =(Select MAX(B2.BorrowDate) 
from Borrow B2 
Where B2.MemberID=B1.MemberID);

Select A.AurthorID, A.Name
from Author A
Join BookAuthor BA ON A.AurthorID=BA.AuthorID
Group By A.AurthorID, A.Name
Having Count(DISTINCT BA.ISBN)>3;

Select DIstinct M.*
From Member M 
Join Borrow B ON M.MemberID=B.MemberID
Join Book BK ON B.ISBN=BK.ISBN
Where BK.YearOfPublication>2015;

Select B.Title, count(*) as BorrowCoubt
From Borrow BR 
...
Join Author A ON BA.AuthorID=A.AurthorID
Where A.Name='Leo Tolstoy';

Select B.*, A.Name AS AuthorName
From Book B
Left Join BookAuthor BA ON B.ISBN=BA.ISBN 
Left Join Author A on A.AurthorID=BA.AuthorID;

Select M.*
From Member M
WHERE(Select Count(*) From Borrow B Where B.MemberID=M.MemberID)>
(Select AVG(BorrowCount) From(Select Count(*) AS BorrowCount From Borrow Group By MemberID) AS AvgCounts);

Select M.MemberID,M.Name,MAX(B.BorrowDate) AS MRB
From Member M Join Borrow B ON M.MemberID=B.MemberID
Group By M.MemberID, M.Name;

Select M.* From Member M
Where( Select Count(Distinct BK.Publisher)
From Borrow B
Join Book BK ON B.ISBN=BK.ISBN
Where B.MemberID=M.MemberID)>1
...
Select A.*
From Author A
Left Join BookAuthor BA ON A.AurthorID=BA.AuthorID
LefT Join Borrow BR ON BA.ISBN=BR.ISBN
...
Select * From Borrow BR Where BR.ISBN=B.ISBN AND BR.MemberID=M.MemberID)
);
